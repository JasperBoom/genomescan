...
# -----------------------------
# Snakefmt formatter targets
# -----------------------------
.PHONY: snakefmt
snakefmt: ## Run snakefmt formatter on Snakemake files.
	@echo "Running snakefmt on Snakemake files..."
	@apptainer exec \
		--containall \
		--bind ${PWD}:/src \
		docker://${SNAKEFMT_IMG}:${SNAKEFMT_IMG_TAG} \
		snakefmt \
			--line-length 80 \
			--verbose \
			/src